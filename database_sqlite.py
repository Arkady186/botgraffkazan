...
async def get_stats() -> dict:
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute("SELECT COUNT(*) as total FROM reports")
        total = (await cursor.fetchone())[0]
        cursor = await db.execute("SELECT COUNT(*) as new FROM reports WHERE status = 'new'")
        new = (await cursor.fetchone())[0]
        cursor = await db.execute("SELECT COUNT(*) as in_progress FROM reports WHERE status = 'in_progress'")
        in_progress = (await cursor.fetchone())[0]
        cursor = await db.execute("SELECT COUNT(*) as completed FROM reports WHERE status = 'completed'")
...
async def get_all_markers() -> list:
...
async def test_connection():
    """Тест подключения к базе (для SQLite просто проверяем файл)."""
    import os
    if os.path.exists(DATABASE_PATH):
        print(f"SQLite файл существует: {DATABASE_PATH}")
        return True
    else:
        print(f"SQLite файл отсутствует, будет создан при init_db")
        return True
